...
exports.runActionCli = function runActionCli (actionName, args, callback) {
  ...
  var actionCliRun = function (action, currentPrefs, commander, tracker, callback) {
    action.cliRun(currentPrefs, commander, tracker, callback);
  };
    ...
    self.getAction(actionName, function (err, action) {
      ...
      if (action.requiresInit) {
        ...
        tracker.verify(function (err) {
          ...
          actionCliRun(action, currentPrefs, commander, tracker, callback);
        });
      } else {
        actionCliRun(action, currentPrefs, commander, null, callback);
      }
    });
  };
